...
function show_profile(){
  ...
  $query = "SELECT * FROM teachers WHERE id = '{$_SESSION['id']}' ;";
  ...
  <p id=''>Institute Graduated From: {$result['graduate_institute']}</p>
  ...
  <p>Knowledge On:<br>1.{$result['sub1']}   2.{$result['sub2']}   3.{$result['sub1']}
  <section>
  ";
  mysqli_close($conn);
...
function show_students(){
  $conn = mysqli_connect("localhost", "aviral", "", "wt_project");
  $q1 = "SELECT id_student FROM following_teachers WHERE id_teacher = {$_SESSION['id']};";
  $q1r = mysqli_query($conn, $q1);
  $id_arr = array();
  while($r1 = mysqli_fetch_assoc($q1r))
    $id_arr[] = $r1["id_student"];

  $len = sizeof($id_arr);
  for($i=0; $i<$len; $i++){
      $query = "SELECT * FROM students WHERE id = {$id_arr[$i]} ;";
    $result = mysqli_fetch_assoc(mysqli_query($conn, $query));
  echo "
  <section id='students'>
  <p id=''>Name : {$result['name']}</p> <p id=''>User Name: {$result['uname']}</p>
...
function show_books(){
  $conn = mysqli_connect("localhost", "aviral", "", "wt_project");
  $q1 = "SELECT id_book FROM following_books WHERE id_teacher = {$_SESSION['id']};";
  $q1r = mysqli_query($conn, $q1);
  $id_arr = array();
  while($r1 = mysqli_fetch_assoc($q1r))
    $id_arr[] = $r1["id_book"];
...
<!--
  ...
  function logout(){
    if(window.location == "teacher.php")
...
<nav id="mySidenav" class="sidenav">
  ...
  <a href="javascript:void(0)" onclick="show_div('books','students','recommendation','search','profile')">Books</a>
  <a href="javascript:void(0)" onclick="show_div('students','books','recommendation','search','profile')">Students</a>
  <a href="javascript:void(0)" onclick="show_div('books','students','recommendation','search','profile')">Books</a>
  <a href="javascript:void(0)" onclick="show_div('recommendation','books','students','search','profile')">Recommendations</a>
  <a href="javascript:void(0)" onclick="show_div('profile', 'books','students','search','recommendation')">Profile</a>
  <a href="javascript:void(0)" onclick="show_div('search','profile', 'books','students','recommendation')">Search</a>
</nav>
...
<body>
    ...
    <article id="students" style="display:none">
